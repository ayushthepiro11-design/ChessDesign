    ...
    return (
      <div className="flex flex-col items-center justify-center gap-6 py-8 w-[440px] max-w-full min-w-0 mx-auto">
        ...
        <div className="text-center space-y-1.5">
          ...
          <div className="text-[12.5px] text-muted dark:text-muted-dark min-h-[16px]">
            {statusText}
          </div>
        </div>
        ...
        {events.length > 0 && (
          <div className="w-full max-w-sm rounded-xl border-2 border-line dark:border-line-dark bg-canvas/60 dark:bg-canvas-dark/60 backdrop-blur overflow-hidden shadow-sm">
            ...
            <ul className="px-3 py-2 font-mono text-[10.5px] leading-relaxed space-y-1 max-h-28 overflow-auto">
              {events.map((e, i) => (
                <li key={i} className="flex items-center gap-2 text-muted dark:text-muted-dark">
                  <span
                    className={[
        ...
        <div className="flex items-center justify-center gap-1.5">
          {[0, 1, 2, 3].map((i) => (
            <span key={i} className="h-1 w-6 rounded-full bg-chip dark:bg-chip-dark overflow-hidden">
              <span className="block h-full bg-ink dark:bg-ink-dark" />
            </span>
          ))}
        </div>
      </div>
